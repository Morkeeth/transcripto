...
# stdlib venv may lack ensurepip on cloud images; fall back to virtualenv.
rm -rf "$WORK/venv"
set +e
python3 -m venv "$WORK/venv" 2>"$WORK/venv.err"
ve_rc=$?
set -e
if [ "$ve_rc" -ne 0 ] || ! "$WORK/venv/bin/python" -c 'import pip' 2>/dev/null; then
  rm -rf "$WORK/venv"
  python3 -m virtualenv "$WORK/venv"
  echo "venv_backend: virtualenv-fallback"
else
...
if printf '%s\n' "$CAVEAT_PROBE" | grep -Fq 'File retention is a find/mtime question'; then
  ...
  echo "pip_only_stats_caveat: ABSENT — published 0.2.0 lacks tip anti-conflation caveat (embarrassment)"
fi

# Beyond-floor (tonight): published package stranger-verb gap vs tip.
VERB_GAP=$(cd "$WORK" && python - <<'PY'
import inspect, re
import transcripto
...
if [ -n "$VERB_GAP" ]; then
  echo "pip_only_verb_gap: DETECTED — missing $VERB_GAP (README stranger flow needs source/tip)"
else
  echo "pip_only_verb_gap: none"
fi
