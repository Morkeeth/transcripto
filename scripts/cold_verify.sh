...
#
# Ambition beyond planter/assert (tonight 2026-09-10; floor = 2026-09-09 wave):
...
#   8. DEATH-RATE IMPOSSIBILITY — two frozen stamps must not print a daily
#      deletion rate (total rose; aging ≠ death).
#   9. EMPTY-GREP control — `grep -q` on empty input exit 1 is not "clean".
#  10. DOCS SWEEP-LAG — deletion is a background sweep after session start;
#      Desktop default 0 = no age limit (article must not flatten that).
#  11. PYPI PACKAGE AUDIT — published wheel has no cold_verify; pip-only
#      cannot reproduce the retention method script.
#
...
# A deletion-day claim needs the corpus to shrink, or old30 to fall because
# files disappeared. Total rose → net additions. old30 rose → aging into the
# bucket and/or already-old arrivals. Neither delta is a death count.
...
# --- EMPTY-GREP control (green-on-outage class) ---
# `printf '' | grep -q .` exits 1. A check that treats "grep found nothing" as
# CLEAN without also proving the input stream was non-empty is the outage trap.
...
if [ "$empty_grep_rc" -eq 0 ]; then
  echo "empty_grep_control: FAIL — grep -q matched empty input"
  FAIL=1
  OFFLINE_CORE_FAIL=1
else
  echo "empty_grep_control: PASS — exit 1 on empty; callers must not treat that as CLEAN without a non-empty guard"
fi
echo
...
rng = random.Random(20260910)  # tonight's seed; not the retention quote
n = 180
...
if [ "$docs_rc" -eq 0 ] && [ -n "$DOCS_MD" ]; then
  ...
  # Bigger object than the Default line: how deletion actually runs.
  if grep -q 'background sweep after a session starts' "$DOCS_FILE"; then
    echo "docs_sweep_lag: PASS — deletion is a background sweep after session start, not continuous"
    echo "docs_sweep_lag_ruling: files can sit past cleanupPeriodDays until the next session starts"
  else
    echo "docs_sweep_lag: NOT FOUND in settings-reference.md tonight — re-open the object before claiming continuous deletion"
  fi
  if grep -qi 'keeps those transcripts at any age\|sets no age limit' "$DOCS_FILE"; then
    echo "docs_desktop_immortal_default: PASS — Desktop/Cowork default keeps transcripts at any age"
    echo "docs_desktop_immortal_ruling: do not flatten '30-day deletion' onto Desktop sessions without checking the mix"
  else
    echo "docs_desktop_immortal_default: NOT FOUND tonight"
  fi
else
...
# PyPI package audit: does the published wheel ship scripts/cold_verify.sh?
# If not, `pip install transcripto` alone cannot reproduce the retention method.
echo
echo "=== PYPI PACKAGE AUDIT (published wheel vs repo stranger path) ==="
set +e
python -m pip download --no-deps --no-cache-dir -d "$WORK/pypi-audit" 'transcripto==0.2.0' -q 2>"$WORK/pypi-audit.err"
dl_rc=$?
set -e
if [ "$dl_rc" -eq 0 ]; then
  WHEEL=$(ls "$WORK/pypi-audit"/transcripto-*.whl 2>/dev/null | head -1)
  echo "pypi_wheel: $WHEEL"
  if [ -n "$WHEEL" ]; then
    set +e
    python - <<PY
import zipfile
...
names = z.namelist()
print("pypi_wheel_entries: %d" % len(names))
hits = [n for n in names if "cold_verify" in n or n.endswith("scripts/cold_verify.sh")]
print("pypi_wheel_cold_verify_hits: %d" % len(hits))
for h in hits[:5]:
    print("  hit:", h)
if hits:
    print("pypi_package_audit: FAIL — published wheel unexpectedly ships cold_verify")
    raise SystemExit(1)
print("pypi_package_audit: PASS — published wheel has no cold_verify script")
print("pypi_package_audit_ruling: stranger retention method requires the repo (clone/archive), not pip-only")
PY
    audit_rc=$?
    set -e
    ...
    echo "pypi_package_audit: SKIP — wheel file missing after download"
  fi
else
  echo "pypi_package_audit: SKIP (pip download exit $dl_rc)"
fi
echo
