...
#
# Ambition beyond planter/assert (tonight 2026-09-14):
...
#  15. MTIME+N vs ! -newermt — naive -mtime +30 is a different cut (watched).
#  16. HARDLINK INFLATION — -type f still counts each name; inode-unique differs.
#
...
# --- HARDLINK INFLATION (-type f still counts each directory entry) ---
# Symlink trap is fixed by -type f. Hardlinks remain: two names, one inode,
# find -type f counts 2. A stranger quoting "file count" without inode-unique
# can inflate totals on trees that hardlink transcripts.
echo "=== HARDLINK INFLATION TRAP (-type f still double-counts names) ==="
HL="$WORK/hardlink-inflation"
mkdir -p "$HL"
printf '%s\n' '{"k":1}' > "$HL/real.jsonl"
ln "$HL/real.jsonl" "$HL/also.jsonl"
HL_TYPEF=$(find "$HL" -type f -name '*.jsonl' | wc -l | tr -d ' ')
HL_INODE=$(find "$HL" -type f -name '*.jsonl' -printf '%i\n' | sort -u | wc -l | tr -d ' ')
echo "hardlink_find_type_f: $HL_TYPEF"
echo "hardlink_unique_inodes: $HL_INODE"
if [ "$HL_TYPEF" = "2" ] && [ "$HL_INODE" = "1" ]; then
  echo "hardlink_inflation: DETECTED — -type f counted 2 names / 1 inode"
  echo "hardlink_inflation_ruling: article method counts names; inode-unique is a different object"
  echo "hardlink_inflation_probe: PASS"
else
  echo "hardlink_inflation: FAIL — expected type_f=2 inodes=1 (got type_f=$HL_TYPEF inodes=$HL_INODE)"
  FAIL=1
  OFFLINE_CORE_FAIL=1
fi
echo

# --- MTIME+N vs calendar ! -newermt (naive two-hour baseline trap) ---
# Competent teams often write `find … -mtime +30`. That is NOT the article
# method (`! -newermt $(date -d '30 days ago' +%Y-%m-%d)`). Plant ages that
# sit between the two cuts and watch disagreement. If none disagree tonight,
# still print the ruling — absence is clock-local.
echo "=== MTIME+N vs CALENDAR ! -newermt (naive baseline trap) ==="
MT="$WORK/mtime-vs-newermt"
mkdir -p "$MT"
NOW_EPOCH=$(date +%s)
# Hours ago spanning the calendar midnight cut vs floor((age_s)/86400) > 30.
MT_DISAGREE=0
MT_SAMPLES=0
for hours in 700 720 721 722 723 724 730 740 750 760; do
  f="$MT/h${hours}.jsonl"
  printf '%s\n' '{"k":1}' > "$f"
  touch -d "@$((NOW_EPOCH - hours * 3600))" "$f"
  bn=$(find "$MT" -name "h${hours}.jsonl" ! -newermt "$THIRTY" | wc -l | tr -d ' ')
  m30=$(find "$MT" -name "h${hours}.jsonl" -mtime +30 | wc -l | tr -d ' ')
  MT_SAMPLES=$((MT_SAMPLES + 1))
  if [ "$bn" != "$m30" ]; then
    MT_DISAGREE=$((MT_DISAGREE + 1))
    echo "mtime_trap_sample: hours_ago=$hours bang_newermt=$bn mtime_plus_30=$m30 DISAGREE"
  else
    echo "mtime_trap_sample: hours_ago=$hours bang_newermt=$bn mtime_plus_30=$m30"
  fi
done
echo "mtime_trap_samples: $MT_SAMPLES"
echo "mtime_trap_disagreements: $MT_DISAGREE"
echo "threshold_30d_calendar: $THIRTY"
if [ "$MT_DISAGREE" -gt 0 ]; then
  echo "mtime_vs_newermt: DETECTED — $MT_DISAGREE/$MT_SAMPLES samples disagree"
  echo "mtime_vs_newermt_ruling: -mtime +30 is NOT the article cut; do not substitute"
  echo "mtime_vs_newermt_probe: PASS"
else
  echo "mtime_vs_newermt: none tonight at this clock — still forbid substituting -mtime +30"
  echo "mtime_vs_newermt_probe: PASS"
fi
echo
...
rng = random.Random(20260914)  # reproducible tonight; not the retention quote
n = 180
...
print("(seed Random(20260914) — not the retention quote; collision guard active)")
