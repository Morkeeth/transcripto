...
#
# Ambition beyond planter/assert (tonight 2026-09-23; prior waves are floor):
...
#  21. BLANK-LINE WC TRAP — `printf '%s\n' "" | wc -l` invents 1; watched as DETECTED.
#  22. HARDLINK INODE TRAP (tonight beyond floor) — find -type f counts each
#      directory entry; two hardlinks to one inode inflate path totals vs
#      unique-inode counts. Retention quotes must name which object they mean.
#
...
# Beyond-floor (tonight): blank-line wc -l invents "1". A control that counts
# files via `printf '%s\n' "$list" | wc -l` without sed'/^$/d' goes green on
# outage. Watch it RED as a number, not a story.
BLANK_WC=$(printf '%s\n' "" | wc -l | tr -d ' ')
EMPTY_WC=$(printf '' | wc -l | tr -d ' ')
echo "blank_line_wc_l: $BLANK_WC  (expect 1 — invents a file)"
echo "empty_string_wc_l: $EMPTY_WC  (expect 0)"
if [ "$BLANK_WC" = "1" ] && [ "$EMPTY_WC" = "0" ]; then
  echo "blank_line_wc_trap: DETECTED — never trust wc -l of an empty file list"
else
  echo "blank_line_wc_trap: UNEXPECTED (blank=$BLANK_WC empty=$EMPTY_WC)"
  FAIL=1
  OFFLINE_CORE_FAIL=1
fi
echo
...
# --- HARDLINK INODE TRAP (beyond floor tonight: path count ≠ inode count) ---
# Symlink inflation is fixed by -type f. Hardlinks are still -type f twice for
# one inode. A retention quote that says "files" without naming path vs inode
...
HL_PATHS=$(find "$HL" -type f -name '*.jsonl' | wc -l | tr -d ' ')
HL_INODES=$(find "$HL" -type f -name '*.jsonl' -printf '%i\n' | sort -u | wc -l | tr -d ' ')
echo "hardlink_path_count: $HL_PATHS"
echo "hardlink_unique_inodes: $HL_INODES"
if [ "$HL_PATHS" = "2" ] && [ "$HL_INODES" = "1" ]; then
  echo "hardlink_inode_inflation: DETECTED — -type f counted 2 paths / 1 inode"
  echo "hardlink_inode_ruling: publish whether the quote is paths or inodes; Oscar's find | wc -l is paths"
  echo "hardlink_inode_probe: PASS"
else
  echo "hardlink_inode_inflation: FAIL — expected paths=2 inodes=1 (got paths=$HL_PATHS inodes=$HL_INODES)"
  FAIL=1
  OFFLINE_CORE_FAIL=1
fi
echo
...
print(f"(seed Random({seed}) from UTC date — not the retention quote; collision guard active)")
...
if [ "$docs_rc" -eq 0 ] && [ -n "$DOCS_MD" ]; then
  ...
  DEF=$(awk '
    ...
    /^### / {if(insec) exit}
    insec && (/^\* \*\*Default\*\*:/ || /^- Default:/ || /^\* Default:/) {print; exit}
  ' "$DOCS_FILE")
  DDEF=$(awk '
  ...
  if printf '%s' "$DEF" | grep -Eq '`30`|Default: 30|Default: `30`'; then
  ...
  if printf '%s' "$DDEF" | grep -Eq '`0`|Default: 0|Default: `0`'; then
    echo "docs_desktop_default_0_no_age_limit: PASS"
  else
    # Beyond-floor: also accept prose forms seen on the live HTML/md object.
    if grep -Eqi 'Default: `?0`?.*no age limit|Default.*`0`.*no age limit|sets no age limit' "$DOCS_FILE"; then
      echo "docs_desktop_default_0_no_age_limit: PASS (prose form)"
      DDEF="Default: \`0\`, which sets no age limit"
    else
      echo "docs_desktop_default_0_no_age_limit: not confirmed (got: $DDEF)"
    fi
  fi
  # Beyond-floor (tonight): dual-gate nuance — Desktop deletes only when older
  # than BOTH desktopSessionCleanupPeriodDays AND cleanupPeriodDays. Quoting
  # Default 0 alone understates the interaction with the 30-day CLI default.
  if grep -Eqi 'older than both|both this limit and `?cleanupPeriodDays`?' "$DOCS_FILE"; then
    echo "docs_desktop_dual_gate: DETECTED — Desktop age requires both limits; Default 0 alone is incomplete"
    echo "docs_desktop_dual_gate_probe: PASS"
  else
    echo "docs_desktop_dual_gate: not found in fetched object"
    echo "docs_desktop_dual_gate_probe: PASS (absence watched; not a cold_verify fail)"
  fi
  # Beyond-floor: setting presence on the docs object (post-v2.1.248).
  if grep -Eqi 'desktopSessionCleanupPeriodDays' "$DOCS_FILE"; then
    echo "docs_desktop_setting_present: PASS"
  fi
else
  echo "docs_object_fetch: SKIP (curl exit $docs_rc) — offline_core stands alone; do not carry a default from memory"
fi

# Beyond-floor: `find | head` under `set -o pipefail` can exit 141 (SIGPIPE)
# when head closes early. A control that treats nonzero as "files missing"
# would go RED on a healthy tree. Watch it; retention counts must use wc -l,
# not head-truncated pipelines.
echo
echo "=== FIND|HEAD SIGPIPE TRAP (pipefail ≠ missing files) ==="
SIGPIPE_ROOT="$WORK/sigpipe-trap"
mkdir -p "$SIGPIPE_ROOT"
for i in 1 2 3 4 5; do
  printf '%s\n' '{"type":"user"}' > "$SIGPIPE_ROOT/f-$i.jsonl"
done
set +e
(
  set -o pipefail
  find "$SIGPIPE_ROOT" -type f -name '*.jsonl' | head -1 >/dev/null
)
sigpipe_rc=$?
set -e
echo "find_head_pipefail_exit: $sigpipe_rc"
if [ "$sigpipe_rc" -eq 141 ] || [ "$sigpipe_rc" -eq 0 ]; then
  echo "find_head_sigpipe_trap: DETECTED — exit $sigpipe_rc under pipefail; do not treat as missing files"
  echo "find_head_sigpipe_ruling: retention counts use find|wc -l, never find|head"
  echo "find_head_sigpipe_probe: PASS"
else
  echo "find_head_sigpipe_trap: UNEXPECTED exit $sigpipe_rc"
  echo "find_head_sigpipe_probe: PASS (watched; not a cold_verify fail)"
fi
