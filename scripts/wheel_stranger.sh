...
rm -rf "$WORK/build-venv"
set +e
python3 -m venv "$WORK/build-venv" 2>/dev/null
bve_rc=$?
set -e
if [ "$bve_rc" -ne 0 ] || ! "$WORK/build-venv/bin/python" -c 'import pip' 2>/dev/null; then
  rm -rf "$WORK/build-venv"
  python3 -m virtualenv "$WORK/build-venv"
fi
...
ive_rc=$?
set -e
if [ "$ive_rc" -ne 0 ] || ! "$VENV/bin/python" -c 'import pip' 2>/dev/null; then
  rm -rf "$VENV"
