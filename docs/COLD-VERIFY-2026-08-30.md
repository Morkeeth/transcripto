# Cold verify — 2026-08-30 artifact (re-run 2026-09-21)
...
**Cold clone stranger (fresh `git clone` of this tip, ≠ worktree):**
...
- Dual-gate: Desktop deletes only when older than **both** Desktop and CLI
  cleanup periods — Default `0` alone is incomplete (OQ-4)
- Desktop/Cowork transcripts are kept at any age by default — do not flatten
  "30-day deletion" onto Desktop sessions without checking the live mix
...
tz_la_threshold: 2026-08-21 counted=0
...
tz_pair_trap: DETECTED — UTC↔Tokyo agree (2026-08-22) while LA differs (2026-08-21)
...
| command | result |
|---------|--------|
| `bash scripts/archive_stranger.sh` | `archive_has_git: no` · `cold_verify: PASS` |
| `bash scripts/wheel_stranger.sh` | `cwd_shadow_trap: DETECTED` · `wheel_stranger: PASS` |
| `bash scripts/cold_clone_stranger.sh` | clone ≠ worktree · `cold_verify: PASS` |
| `bash scripts/pip_only_baseline.sh` | naive find wins · `pip_only_baseline: PASS` |
| `./test_small_n.sh` / `bash scripts/test_small_n.sh` | **7/7 green** |
| `./test_privacy_empty_index.sh` | watched RED · PASS |
| `python3 -m unittest discover -s tests -q` | **94 OK** |

Oracle tonight (re-derived, seed `20260921`): old30=**87**, old45=**40**.
TZ tonight: **divergence OBSERVED** (LA cut `2026-08-21` vs UTC/Tokyo/London
`2026-08-22`); publish TZ with any live figure.
