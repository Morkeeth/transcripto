...
5. **Empty-index privacy** — watched RED (exit 1); main used to go green with
   blank-line `wc -l` inventing "1 file".
...
8. **Frozen quotes FAIL-TO-RECONCILE** — no unique daily death rate from
   504/2721 → 579/2874.
9. **TZ divergence: none tonight** — UTC/LA/Tokyo/London all cut at
   `2026-08-20` at this clock. Still publish TZ with live figures. (Prior
   waves saw a pair trap at other clocks; tonight does not invent one.)
10. **Desktop dual-gate DETECTED** — docs object
    (`settings-reference.md`) says Default `0` (no age limit) **and** that
    Desktop deletes only when older than **both**
    `desktopSessionCleanupPeriodDays` and `cleanupPeriodDays` (Default 30).
    Quoting Default 0 alone is incomplete.
11. **`find | head` under pipefail** — exit 0 tonight on this host; still
    watched so a SIGPIPE 141 is never read as "files missing".
...
(seed Random(20260919) from UTC date — not the retention quote; collision guard active)
...
tz_la_threshold: 2026-08-20 counted=0
tz_tokyo_threshold: 2026-08-20 counted=0
tz_london_threshold: 2026-08-20 counted=0
tz_local_threshold: 2026-08-20
tz_divergence: none at this clock (all four thresholds: 2026-08-20); still document TZ with live figures
...
- `bash scripts/archive_stranger.sh` → `cold_verify: PASS` (ARCHIVE_EXIT 0)
- `bash scripts/wheel_stranger.sh` → `wheel_stranger: PASS`
  (`cwd_shadow_trap: DETECTED`)
- `bash scripts/cold_clone_stranger.sh` → `cold_verify: PASS`
- `bash scripts/pip_only_baseline.sh` → `pip_only_baseline: PASS`
  (`arm_a_ruling: naive find WINS retention`)
- `python3 -m unittest discover -s tests -q` → **94 OK**
- `./test_small_n.sh` and `bash scripts/test_small_n.sh` → **7/7 green**
