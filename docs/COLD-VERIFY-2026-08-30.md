# Cold verify artifact — 2026-09-17
...
Tip: `0b1cc06` on `cursor/night-wave-p1-cold-verify-4eac`. Host `TZ=UTC`.
...
4. **`venv_partial_without_pip: DETECTED`** — `python3 -m venv` exit 1 still left
...
6. **Wheel cwd-shadow DETECTED** — importing from repo cwd binds the tree.
7. **Frozen quotes FAIL-TO-RECONCILE** — no unique daily death rate from
   504/2721 → 579/2874.

## Command output (tonight, RUN)

Host `TZ=UTC`. Final re-run after docs landed on HEAD (`0b1cc06`). Re-derived
figures — do not carry prior-wave oracle counts.

```
=== COLD VERIFY · 2026-09-17T08:21:43Z ===
repo: /workspace
...
mode: fixture
as_of: 2026-09-17T08:21:49Z
threshold_30d: mtime <= 2026-08-18
threshold_45d: mtime <= 2026-08-03
total_jsonl: 2721
older_than_30d: 504
older_than_45d: 0
...
oldest_mtime: 2026-08-04T08:21:45Z
...
threshold_30d_epoch: 1787011200.0
threshold_45d_epoch: 1785715200.0
independent_oracle: PASS
(seed Random(20260917) — not the retention quote; collision guard active)

=== BOUNDARY PROBE (exactly at threshold) ===
threshold_midnight_epoch: 1787011200 (2026-08-18 00:00:00 local)
...
calendar_only: 0
duration_only: 0
ref_calendar_epoch: 1787011200
ref_duration_epoch: 1787041310
method_skew_seconds: 30110
...
threshold_30d: 2026-08-18
touch_utime_planter: PASS — find counts agree (7)

=== TZ DIVERGENCE PROBE (multi-zone calendar cut) ===
tz_utc_threshold: 2026-08-18 counted=0
tz_la_threshold: 2026-08-18 counted=0
tz_tokyo_threshold: 2026-08-18 counted=0
tz_london_threshold: 2026-08-18 counted=0
tz_local_threshold: 2026-08-18
tz_divergence: none at this clock (all four thresholds: 2026-08-18); still document TZ with live figures
...
| Pip-only baseline | `bash scripts/pip_only_baseline.sh` | **PASS** (naive find wins; verb gap DETECTED) |
| STEP 0 | `./test_small_n.sh` + `bash scripts/test_small_n.sh` | **7/7** both |
| Units | `python3 -m unittest discover -s tests -q` | **94 OK** |
