...
cold verify green 2026-09-16 ~19:00Z · article / X = **Oscar clicks only**
...
| Archive stranger | `bash scripts/archive_stranger.sh` | **PASS** (no `.git`; privacy ephemeral index) |
| Wheel stranger | `bash scripts/wheel_stranger.sh` | **PASS** — `cwd_shadow_trap: DETECTED`; fixture 504/2721 PASS |
| Cold clone | `bash scripts/cold_clone_stranger.sh` | **PASS** — clone ≠ `/workspace` |
| Cold artifact | `docs/COLD-VERIFY-2026-08-30.md` | captured command output (2026-09-16 ~19:00Z) |
| Baseline arm | `docs/BASELINE-ARM.md` + `bash scripts/pip_only_baseline.sh` | naive `find` wins retention; pip-only cannot answer file-age; **0.2.0 stats caveat ABSENT** |
...
| Unit tests | `python3 -m unittest discover -s tests -v` | **94 OK** |
| Docs default | curl settings-reference.md | `cleanupPeriodDays` Default **30**; desktop Default **0** |
| PyPI | `curl -sL https://pypi.org/pypi/transcripto/json` | **0.2.0**; wheel cold_verify entries **0**; tip caveat missing on live wheel |
| Stranger journey | inside cold_verify | `stranger_product_journey: PASS` (ask Open×3, brief mode 600) |
...
5. [ ] PyPI publish only if a new version is intentionally cut (current source is 0.2.0;
   tip stats caveat is not on the live wheel until a release that includes it).

## Footer — night wave 2026-09-16 log
...
… independent_oracle: PASS (old30=92, old45=44; seed 20260916)
… boundary_probe: PASS
… duration_vs_calendar: … counted_by_bang_newermt=0
… calendar_duration_delta: none tonight (fixture ages 31–44d)
… touch_utime_planter: PASS (7 of 10)
… tz_divergence: OBSERVED (UTC/LA/London=2026-08-17; Tokyo=2026-08-18)
… tz_pair_trap: DETECTED — UTC↔LA agree while Tokyo differs
...
… pypi_package_audit: PASS (wheel cold_verify entries=0)
...
$ # Intermediate FAIL (RUN): published_stats probe first saved wheel as
$ #   pypi-wheel.whl (pip rejected filename); fixed to basename from URL.
...
$ # Intermediate FAIL (RUN): python3 -m venv without ensurepip; cold_verify
$ #   now falls back to virtualenv and refuses a pip-less bin/python.
```
