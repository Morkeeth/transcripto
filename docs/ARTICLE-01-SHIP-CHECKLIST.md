...
cold verify green 2026-09-14 ~00:10Z · article / X = **Oscar clicks only**
...
| Baseline arm | `docs/BASELINE-ARM.md` + `bash scripts/pip_only_baseline.sh` | naive `find` wins retention; `-mtime +30` disagrees; pip-only cannot answer file-age; gate 8 vs naive 12 |
...
1. [ ] Re-derive live retention on Oscar's machine (optional but honest):
   ...
   (stamps FAIL-TO-RECONCILE). **Do not use `-mtime +30`.** Note Desktop mix
   (desktop cleanup Default 0).
...
## Footer — night wave 2026-09-14 log
...
$ bash test_privacy_empty_index.sh
