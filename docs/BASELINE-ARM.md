...
Measured **2026-09-16**. Numbers re-derived at their objects; not carried from
...
# → published_stats_caveat: ABSENT on live 0.2.0; tip_stats_caveat: PRESENT
...
1. Quoting `stats` `2,721` as retention evidence — **DETECTED** on source,
   ...
   Tip repair: stats prints an explicit caveat that these are indexed message
   counts, not transcript file ages (`stats_anti_conflation_caveat: PASS`).
   **Live PyPI 0.2.0 still lacks that caveat** (`published_stats_caveat: ABSENT`).
...
5. `python3 -m venv` without ensurepip — watched; fell back to virtualenv
   rather than painting a broken tree green.
