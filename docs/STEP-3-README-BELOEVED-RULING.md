...
**Cloud recommendation (2026-09-06): KEEP the current invented replay demo.**
...
rg -n 'invented' README.md
```

Observed at `README.md:64–88` (via `nl -ba README.md | sed -n '64,90p'`):

- Heading `## The replay` at L64
- Explicit label at L66–67: all prompts and results in the example are **invented**
- Demo block L69–88: `THE COMEBACK · claude · request 1` with
  `You asked: "Fix the login redirect and run its tests."`
- Closing: `Recorded: 3 succeeded · 2 failed · 0 unknown`

`rg` for old author-prompt markers (`beloeved|NO-DURABLE|worst looped`) in
`README.md`: **0 hits**.
